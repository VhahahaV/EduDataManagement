...
const ClassTeacherPage = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'process',
      key: 'process',
      render: (text, record) => <span style={{ fontFamily: "'Comic Sans MS', 'Comic Sans', cursive" }}>{`${text}/${record.total}`}</span>
    }
  ];
  ...
  const calculateAverageScore = (students) => {
    ...
    students.forEach(student => {
      totalScore += student.process / student.total * 100;
    });
  ...
  const calculateCompletionRate = (students) => {
    let completed = 0;
    students.forEach(student => {
      if (student.process === student.total) {
        completed++;
      }
    });
    return (completed / students.length * 100).toFixed(2);
  };

  const calculateExcellentRate = (students) => {
    let excellent = 0;
    students.forEach(student => {
      if ((student.process / student.total * 100) >= 85) {
        excellent++;
      }
    });
  ...
  const completionRate = calculateCompletionRate(students);
  const excellentRate = calculateExcellentRate(students);
              ...
              <Tabs>
                <TabPane tab="课程详情" key="1">
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    <Paragraph>高等数学是高等学校理工科专业重要的基础理论课，是全校性的公共基础课，对于以信息和电子学科为主的各理工科专业，高等数学在大学本科教育阶段显得尤为重要，有着举足轻重的作用。该课程不但是学习复变函数、概率统计、大学物理等课程的必修课，而且为学习专业课程和进一步扩大数学知识奠定必要的数学基础。通过该课程的教学，一是为学生的后继课程教学提供必需的基础数学知识；二是传授数学思想，培养学生的创新意识，逐步提高学生的数学素养、数学思维能力和应用数学知识解决复杂问题的能力。</Paragraph>
        ...
        <Col span={7} style={{ marginTop: "16px" }}>
          <Row>
            <Content style={{
              ...
              <Paragraph>平均分：{averageScore}</Paragraph>
              <Paragraph>课程教师：武忠祥，张宇</Paragraph>
              <Paragraph>优秀率：{excellentRate}%</Paragraph>
              <Paragraph>完成率：{completionRate}%</Paragraph>
            </Content>
          </Row>
          <Row>
            <Content style={{
